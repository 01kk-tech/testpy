import pandas as pd
import plotly.graph_objects as go
import os

# โหลดข้อมูล
df = pd.read_csv('bquxjob_6c32ae3b_19f6dfc7004.csv')
df['sales_date'] = pd.to_datetime(df['sales_date'], format='%d/%m/%Y', errors='coerce')

# ทำความสะอาด time_range
def clean_time_range(t):
    t = str(t)
    if '00-01' in t: return '12AM-1AM'
    elif t.startswith('1-'): return '1AM-2AM'
    elif t.startswith('2-'): return '2AM-3AM'
    elif t.startswith('3-'): return '3AM-4AM'
    elif t.startswith('4-'): return '4AM-5AM'
    elif t.startswith('5-'): return '5AM-6AM'
    elif t.startswith('6-'): return '6AM-7AM'
    elif t.startswith('7-'): return '7AM-8AM'
    elif t.startswith('8-'): return '8AM-9AM'
    elif t.startswith('9-'): return '9AM-10AM'
    elif t.startswith('10-'): return '10AM-11AM'
    elif t.startswith('11-'): return '11AM-12PM'
    elif '13-14' in t: return '1PM-2PM'
    elif '14-15' in t: return '2PM-3PM'
    elif '15-16' in t: return '3PM-4PM'
    elif '16-17' in t: return '4PM-5PM'
    elif '17-18' in t: return '5PM-6PM'
    elif '18-19' in t: return '6PM-7PM'
    elif '19-20' in t: return '7PM-8PM'
    elif '20-21' in t: return '8PM-9PM'
    elif '21-22' in t: return '9PM-10PM'
    elif '22-23' in t: return '10PM-11PM'
    elif '23-24' in t: return '11PM-12AM'
    else: return t

df['time_period'] = df['time_range'].apply(clean_time_range)

time_order = [
    '12AM-1AM','1AM-2AM','2AM-3AM','3AM-4AM','4AM-5AM','5AM-6AM',
    '6AM-7AM','7AM-8AM','8AM-9AM','9AM-10AM','10AM-11AM','11AM-12PM',
    '1PM-2PM','2PM-3PM','3PM-4PM','4PM-5PM','5PM-6PM','6PM-7PM',
    '7PM-8PM','8PM-9PM','9PM-10PM','10PM-11PM','11PM-12AM'
]

os.makedirs('charts', exist_ok=True)

# ============================================================
# Chart 1: Hourly Sales Velocity Combo Chart
# ============================================================
hourly_data = df.groupby('time_period').agg(
    revenue=('sales_amount', 'sum'),
    transactions=('sales_amount', 'count')
).reset_index()
hourly_data['time_period'] = pd.Categorical(hourly_data['time_period'], categories=time_order, ordered=True)
hourly_data = hourly_data.sort_values('time_period')

fig1 = go.Figure()
fig1.add_trace(go.Bar(
    x=hourly_data['time_period'], y=hourly_data['revenue'],
    name="Sales Amount", marker_color='#38B2AC', yaxis='y'
))
fig1.add_trace(go.Scatter(
    x=hourly_data['time_period'], y=hourly_data['transactions'],
    name="Transaction Volume", mode='lines+markers',
    marker=dict(color='#81E6D9', size=8), line=dict(width=3, color='#81E6D9'), yaxis='y2'
))
fig1.update_layout(
    title="Hourly Sales Velocity and Transaction Volume",
    plot_bgcolor='white', height=400,
    yaxis=dict(title="Sales Amount ($)", showgrid=True, gridcolor='#F3F4F6'),
    yaxis2=dict(title="Transactions", overlaying='y', side='right', showgrid=False),
    xaxis=dict(showgrid=False, tickangle=-45),
    hovermode="x unified",
    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
)
fig1.write_html("charts/hourly_sales_velocity.html", include_plotlyjs='cdn', full_html=True)
print("✅ Chart 1: Hourly Sales Velocity (Interactive HTML)")

# ============================================================
# Chart 2: Top Stores by Revenue
# ============================================================
store_data = df.groupby('store_id').agg(
    revenue=('sales_amount', 'sum'),
    transactions=('sales_amount', 'count')
).reset_index().sort_values('revenue', ascending=True)

fig2 = go.Figure()
fig2.add_trace(go.Bar(
    y=store_data['store_id'].astype(str),
    x=store_data['revenue'],
    orientation='h',
    marker=dict(color=store_data['revenue'], colorscale=['#FCA5A5', '#EF4444', '#B91C1C']),
    text=store_data['revenue'].apply(lambda x: f"${x:,.0f}"),
    textposition='auto',
    hovertemplate="<b>Store: %{y}</b><br>Revenue: $%{x:,.2f}<br>Transactions: %{customdata:,}<extra></extra>",
    customdata=store_data['transactions']
))
fig2.update_layout(
    title="Top Stores by Revenue",
    plot_bgcolor='white', height=300,
    xaxis=dict(title="Revenue ($)", showgrid=True, gridcolor='#F3F4F6'),
    yaxis=dict(title="Store ID", showgrid=False, type='category')
)
fig2.write_html("charts/top_stores.html", include_plotlyjs='cdn', full_html=True)
print("✅ Chart 2: Top Stores (Interactive HTML)")

# ============================================================
# Chart 3: Daily Revenue Trend
# ============================================================
daily = df.groupby('sales_date').agg(
    revenue=('sales_amount', 'sum')
).reset_index().sort_values('sales_date')

fig3 = go.Figure()
fig3.add_trace(go.Scatter(
    x=daily['sales_date'], y=daily['revenue'],
    mode='lines+markers',
    name="Daily Revenue",
    line=dict(color='#38B2AC', width=3),
    marker=dict(color='#38B2AC', size=6),
    fill='tozeroy', fillcolor='rgba(56, 178, 172, 0.1)'
))
fig3.update_layout(
    title="Daily Revenue Trend",
    plot_bgcolor='white', height=350,
    xaxis=dict(title="Date", showgrid=False, rangeselector=dict(
        buttons=list([
            dict(count=7, label="7d", step="day", stepmode="backward"),
            dict(count=14, label="14d", step="day", stepmode="backward"),
            dict(count=1, label="1m", step="month", stepmode="backward"),
            dict(count=3, label="3m", step="month", stepmode="backward"),
            dict(step="all")
        ])
    )),
    yaxis=dict(title="Revenue ($)", showgrid=True, gridcolor='#F3F4F6'),
    hovermode="x unified"
)
fig3.write_html("charts/daily_revenue_trend.html", include_plotlyjs='cdn', full_html=True)
print("✅ Chart 3: Daily Revenue Trend (Interactive HTML)")

# ============================================================
# Chart 4: Revenue by Day of Week
# ============================================================
df['day_name'] = df['sales_date'].dt.day_name()
day_order_list = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
dow = df.groupby('day_name').agg(revenue=('sales_amount', 'sum')).reindex(day_order_list).reset_index()

fig4 = go.Figure()
fig4.add_trace(go.Bar(
    x=dow['day_name'], y=dow['revenue'],
    marker_color=['#FCA5A5', '#F87171', '#EF4444', '#DC2626', '#B91C1C', '#991B1B', '#7F1D1D'],
    text=dow['revenue'].apply(lambda x: f"${x:,.0f}"),
    textposition='auto',
    hovertemplate="<b>%{x}</b><br>Revenue: $%{y:,.2f}<extra></extra>"
))
fig4.update_layout(
    title="Revenue by Day of Week",
    plot_bgcolor='white', height=350,
    xaxis=dict(title="Day", showgrid=False),
    yaxis=dict(title="Revenue ($)", showgrid=True, gridcolor='#F3F4F6')
)
fig4.write_html("charts/revenue_by_day.html", include_plotlyjs='cdn', full_html=True)
print("✅ Chart 4: Revenue by Day of Week (Interactive HTML)")

print()
print("=" * 50)
print("✅ สร้าง Interactive HTML Charts ครบ 4 ไฟล์แล้ว!")
print("📁 อยู่ในโฟลเดอร์ charts/")
print("=" * 50)